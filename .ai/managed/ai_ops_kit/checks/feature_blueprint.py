"""Проверяющая логика feature-blueprint, вынесена из `validation/validate_feature_blueprint.py`
вниз (лента №5), чтобы рантайм (lifecycle.run_report) звал её ВНИЗ, без восходящего ребра
lifecycle -> validation.

Валидатор feature-blueprint читает КАТАЛОГ функции (blueprint.yaml, существование артефактов,
DeliveryReceipt, долг доказательства) — это read-only I/O, и разложить его на «чистое check(data) +
I/O у вызывателя» нельзя, не переписав обход. Поэтому логика переезжает сюда целиком: `checks`
держит проверяющую логику НИЖЕ entrypoints, зависит только от stdlib и pyyaml и НЕ импортирует
ничего из ai_ops_kit выше foundation. CLI-обёртка в `validation` ре-экспортирует эти имена.
"""
