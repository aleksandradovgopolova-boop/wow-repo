"""Совместимость: плоское имя ai_route -> ai_ops_kit.engine.ai_route.

v3.34: движок маршрутизации лежал в validation/ и валидатором не был — при переносе
валидаторов в пакет это стало видно. Переехал в engine, где его место по смыслу.
...
`import ai_ops_kit...` падает при запуске файла напрямую (`python3 tools/ai_route.py`)
...
if __name__ == "__main__":
    ...
    runpy.run_module("ai_ops_kit.engine.ai_route", run_name="__main__", alter_sys=True)
else:
    import ai_ops_kit.engine.ai_route as _target
